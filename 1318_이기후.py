...
def sungsoo_wihan_package_downloader(package):
    ...
    assets = [
        ...
        "SFX/Pulse_Effect.wav",
        "SFX/DoomLaserCharge.wav",
        "SFX/DOomLaserSFX.wav",
    ]
    ...
    assets += [f"Sprites/Enemy/Explosive_Robot/Explosive_Robot{i}.png" for i in range(1, 6)]
    ...
    assets += [f"SFX/10km_Howling{i}.mp3" for i in range(1, 6)]
...
pygame.mixer.init()
SOUNDS = {
    "laser": pygame.mixer.Sound("SFX/LaserSFX.wav"),
    "break_platform": pygame.mixer.Sound("SFX/Break_platform.mp3"),
    "hype": pygame.mixer.Sound("SFX/Hype.mp3"),
    "interude": pygame.mixer.Sound("SFX/Interude.mp3"),
    "10km_howling": pygame.mixer.Sound("SFX/10km_Howling5.mp3"),
    "Pulse": pygame.mixer.Sound("SFX/Pulse_Effect.wav"),
    "Boss_Appear": pygame.mixer.Sound("SFX/23M-RFT70_Appear.mp3"),
    "Boss_Teleport": pygame.mixer.Sound("SFX/23M-RFT70_Teleport.mp3"),
    "Boss_TeleportEnd": pygame.mixer.Sound("SFX/23M-RFT70_TeleportEnd.mp3"),
    "Boss_AttackAlarm": pygame.mixer.Sound("SFX/23M-RFT70_AttackAlarm.mp3"),
    "Boss_Death": pygame.mixer.Sound("SFX/23M-RFT70_Death.mp3"),
    "Boss_EdgeShredding": pygame.mixer.Sound("SFX/23M-RFT70_EdgeShredding.mp3"),
    "Boss_CuttingDimension": pygame.mixer.Sound("SFX/23M-RFT70_CuttingDimension.mp3"),
    "Boss_TremblingSlash": pygame.mixer.Sound("SFX/23M-RFT70_TremblingSlash.mp3"),
    "doomlaser_charge": pygame.mixer.Sound("SFX/DoomLaserCharge.wav"),
    "doomlaser_sfx": pygame.mixer.Sound("SFX/DOomLaserSFX.wav"),
}
BGM = {
    "main": "SFX/RE_Venture-Starting.wav", 
    "ingame": "SFX/Radical_Rush.mp3", 
    "higher": "SFX/Eternal_Return_Eternal_Forsaken.mp3", 
    "Boss": "SFX/Experiment_67.mp3"
    }
def play_bgm(key, volume=0.5, start=0.0, loop=True):
...
class LogoShow:
    def __init__(self):
        ...
        play_bgm('main', volume=0.5, start=22.0, loop=True)
...
class ExplosiveRobot(Enemy):
    ...
    def __init__(self, HP, sprite_size, can_death_instantly, x, y):
        ...
        self.explode_speed = 2500

        self.explodeready_timer = 0
        self.explodeready_duration = 1.0
        
        self.target_pos = None

        if ExplosiveRobot.sprite_cash is None:
            ExplosiveRobot.sprite_cash = {
                "idle": [
                    load_sprites(
                        f"Sprites/Enemy/Explosive_Robot/Explosive_Robot{i}.png", sprite_size
                    )
                    for i in range(1, 2)
                ],
                "explodeready": [
        ...
        self.sprites = ExplosiveRobot.sprite_cash
        self.current_animation = "idle"
        self.animation_speed = 0.1

    def update(self, delta, player, camera, pulses=None, platforms=None):
        if self.HP <= 0:
            return
        ...
        if self.state == 'idle':
            self.current_animation = 'idle'
            if dist > 0:
                self.x += (dx / dist) * self.idle_speed * delta
                self.y += (dy / dist) * self.idle_speed * delta

            if dist < 150:
                self.state = 'explodeready'
                self.explodeready_timer = 0
        
        elif self.state == 'explodeready':
            self.current_animation = 'explodeready'
            self.explodeready_timer += delta
            if self.explodeready_timer >= self.explodeready_duration:
                self.target_pos = (player.x + player.width / 2, player.y + player.height / 2)
                self.state = 'explode'

        elif self.state == 'explode':
            self.current_animation = 'explodeready'
            if self.target_pos:
                target_dx = self.target_pos[0] - (self.x + self.width / 2)
                target_dy = self.target_pos[1] - (self.y + self.height / 2)
                target_dist = math.hypot(target_dx, target_dy)

                if target_dist > 20:
                    self.x += (target_dx / target_dist) * self.explode_speed * delta
                    self.y += (target_dy / target_dist) * self.explode_speed * delta
                else:
                    # Explode
                    effect = {"damage": 2, "knockback": (0, -500), "stun_duration": 1.0}
                    pulse = PulseWave(
                        x=self.x + self.width // 2,
                        y=self.y + self.height // 2,
                        initial_radius=10,
        ...
        self.x = max(0, min(self.x, SCREEN_WIDTH - self.width))
        if camera:
             self.y = max(abs(camera.camera_y), min(self.y, abs(camera.camera_y) + SCREEN_HEIGHT - self.height))
...
class RangedRobot(Enemy):
    ...
    def state_cycle(self, delta):
        ...
        if self.timer <= self.idle_time:
            ...
            if self.current_animation != 'shootready':
                SOUNDS['doomlaser_charge'].play()
...
class Boss23M(Enemy):
    ...
    def update(self, delta, player, camera, pulses, lasers, platforms, after_images):
        ...
        if self.state == "stunned":
            if not self.current_animation == "stunned":
                SOUNDS["Boss_Death"].play()
            self.current_animation = "stunned"
            stun_duration = 3.0
    ...
    def pattern_dimension_strike(self, delta, player, camera, pulses, platforms):
        ...
        if self.pattern_step == 0:
            ...
            self.y += self.y_velocity * delta
            SOUNDS["Boss_CuttingDimension"].play()
    ...
    def teleport_player_down(self, player, platforms, num_platforms):
        ...
        target_idx = min(2, len(sorted_platforms) - 1)
        if target_idx >= 0:
    ...
    def pattern_edge_shredding(self, delta, player, camera, pulses):
        ...
        if self.pattern_step == 0:
            ...
            if self.pattern_timer >= 0.05:
                ...
                teleport_y_offset = random.uniform(SCREEN_HEIGHT * 0.7, SCREEN_HEIGHT * 0.9)
                self.y = screen_top + teleport_y_offset
                SOUNDS['Boss_Teleport'].play()
    ...
    def pattern_trembling_slash(self, delta, player, camera, after_images):
        ...
        if self.pattern_step == 0:
            ...
            if self.pattern_timer >= self._dash_duration:
                SOUNDS['Boss_TremblingSlash'].play()
                self._dash_count += 1
...
class EnemySpawner:
    ...
    def generate(self, flag, platforms):
        ...
        if enemy_type_to_spawn == "None" or enemy_type_to_spawn not in self.enemy_data:
            return
        ...
        if spawn_type == 'air':
            ...
            elif enemy_type_to_spawn == 'ExplosiveRobot':
                enemy = ExplosiveRobot(hp, size, instant_deathable, x, y)
...
class Game:
    ...
    def start_boss_fight(self):
        ...
        play_bgm("Boss", start=58.0, loop=True)
        SOUNDS['Boss_Appear'].play()
        SOUNDS['interude'].play()
    ...
    def run(self):
        ...
        if not self.boss_fight_active:
            ...
            if not self.music_changed_flag and self.current_event == "The Howling appeared.":
                self.music_changed_flag = True
                play_bgm('higher', loop=True)
                SOUNDS['10km_howling'].play()
        ...
        if self.player.hitstop_timer <= 0:
            ...
            for enemy in self.enemy_spawner.enemies:
                if isinstance(enemy, FireRobot):
                ...
                elif isinstance(enemy, ExplosiveRobot):
                    enemy.update(delta, self.player, self.camera, self.current_pulses)
                    ...
                    enemy.update(delta, self.player, self.camera)
...
while not current_state == GameState.EXIT:
    ...
    if current_state == GameState.PLAYING:
        ...
        if not game_over:
            next_state = Ingame.run()
            if next_state is not None:
                current_state = next_state
                if next_state == GameState.MENU:
                    Ingame = Game() # Re-initialize the game object for a clean state next time
                    Starter = StartMenu() 
                    camera = Camera((SCREEN_WIDTH, SCREEN_HEIGHT)) # Re-initialize camera
                    play_bgm('main', volume=0.5, start=22.0, loop=True)
        else:
